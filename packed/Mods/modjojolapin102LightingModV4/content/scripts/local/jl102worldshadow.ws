...
state sgcInit in CJL102StateMachine
{
	event OnEnterState(prevState : name)
	{
		parent.jl102LM.initJL102LM(theGame.GetInGameConfigWrapper().GetVarValue('jl102Config', 'envIntEnable'));
...
state sgcCustomWorldShadow in CJL102StateMachine
{
	event OnEnterState(prevState : name)
	{
		parent.jl102LM.custom_world_shadow(theGame.GetInGameConfigWrapper().GetVarValue('jl102Config', 'shadowFilterSize'));
